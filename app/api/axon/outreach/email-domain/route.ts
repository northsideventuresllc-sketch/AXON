...
/** Refresh all connected email domains from Resend. */
export async function GET() {
  try {
    ...
    if (domains.size > 0) {
      settings = await saveOutreachSettings(settings);
    }
...
/** Connect an email — registers domain with Resend and stores DNS records. */
export async function POST(req: Request) {
  try {
    ...
    // Upsert email account if not already present
    ...
    // Trigger verification check (async on Resend side)
    if (sync.domain.status !== 'verified') {
      try {
        ...
        /* verification may fail until DNS propagates */
      }
    }
...
/** Trigger Resend verification for a domain. */
