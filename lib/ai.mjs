...
/**
 * AXON-EVERYWHERE-PROJECT (2026-08-05): try AXON's own local model before any paid/free
 * cloud provider. Returns null on any failure so callers fall through unchanged.
 */
async function axonLocalScanProspect(cfg, prospect) {
  const system = 'You research B2B prospects for Northside Intelligence. Return valid JSON only.';
  const text = await callAxonLocal(cfg.supabaseKey, system, SCAN_PROMPT(prospect));
  if (!text) return null;
  const scan = extractJson(text);
  scan._scan_source = 'axon-local';
  return scan;
}

/**
 * AXON-TIER-SYSTEM (2026-08-20, JB direct order): RunPod (AXON v1) tier — sits between
 * AXON-local and Gemini. No-op (returns null) until RUNPOD_AXON_V1_ENDPOINT/KEY exist.
 */
async function axonV1ScanProspect(cfg, prospect) {
  const system = 'You research B2B prospects for Northside Intelligence. Return valid JSON only.';
  const text = await callAxonV1Cloud(cfg.supabaseKey, system, SCAN_PROMPT(prospect));
  if (!text) return null;
  const scan = extractJson(text);
  scan._scan_source = 'axon-v1-runpod';
  return scan;
}
...
/** AXON-local → RunPod (AXON v1) → Gemini → Haiku → SERP metadata fallback so outreach can still queue drafts. */
export async function scanProspect(cfg, prospect) {
  try {
    const local = await axonLocalScanProspect(cfg, prospect);
    if (local) return local;
  } catch (err) {
    console.warn(`AXON-local scan failed (${err.message}) — trying RunPod (AXON v1)`);
  }

  try {
    const v1 = await axonV1ScanProspect(cfg, prospect);
    if (v1) return v1;
  } catch (err) {
    console.warn(`RunPod (AXON v1) scan failed (${err.message}) — trying Gemini`);
  }
...
Return JSON:
{
  ...
  const text = await callTiered(cfg, system, user);
...
Return JSON: { "email_subject": "...", "email_body": "..." }`;
  const text = await callTiered(cfg, system, user, 600);
  return extractJson(text);
}

/**
 * AXON-EVERYWHERE-PROJECT (2026-08-05): AXON-local -> Gemini -> Haiku last resort.
 * Shared tiered caller for the two functions above (score/draft, follow-up) that
 * previously called paid Haiku with zero fallback of any kind. Decision #598 item 11 / #619.
 *
 * AXON-TIER-SYSTEM (2026-08-20, JB direct order): RunPod (AXON v1) tier inserted here,
 * right after AXON-local and before Gemini, per the canonical org-wide tier order.
 * callAxonV1Cloud is a documented no-op (returns null) until RunPod is deployed, so this
 * is a no-op change until then.
 */
async function callTiered(cfg, system, user, maxTokens = 1200) {
  const local = await callAxonLocal(cfg.supabaseKey, system, user).catch(() => null);
  if (local) return local;

  const runpod = await callAxonV1Cloud(cfg.supabaseKey, system, user).catch(() => null);
  ...
  if (cfg.geminiKey) {
    try {
      ...
      console.warn(`Gemini tiered call failed (${err.message}) — trying Haiku`);
    }
  }

  return callHaiku(cfg.anthropicKey, system, user, maxTokens);
}
