 ...
 * the same picture.
 */
export async function buildJbChatContext(sbSelect, { now = new Date(), pipelineContext = '' } = {}) {
  ...
  const facts = {
    ...
    failures: [needsJb, approvals, fleet, closeOut].filter((r) => !r.ok).length,
  };
  facts.waiting = [...facts.needsJb, ...facts.approvals].sort(
    (a, b) => new Date(a.createdAt || 0) - new Date(b.createdAt || 0),
  );
  facts.text = renderContext(facts, { needsJb, approvals, fleet, closeOut });
  return facts;
}
...
export function answerWhatNeedsJb(facts) {
  ...
  if (!items.length) {
    return facts.failures
      ? "I don't have that in front of me — the job list didn't come back this time. ARCEUS can read it directly."
      : NOTHING_WAITING;
  }
  const head = items.length === 1 ? 'One thing needs you.' : `${items.length} things need you.`;
  return [head, ...items.map((i) => describeWaitingItem(i))].join('\n').slice(0, 4000);
}
