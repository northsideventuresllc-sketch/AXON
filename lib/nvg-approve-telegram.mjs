...
export async function handleNvgApproveCallback(cfg, sb, callbackQuery) {
  ...
  // TELEGRAM-RETIRE-APPROVALS-TOPIC-0924 (Decision #2012): the shared "JB
  // Approvals" group topic is retired — no card is sent there any more (see
  // jb-route.mjs). This inbound check is kept, not removed, purely so a tap
  // on any card JB already has sitting in that old topic from before the
  // retirement still resolves instead of silently 403ing; it does not mean
  // new cards go there. Taps from JB's private chat carry no topic and are
  // accepted as-is; a tap from inside the group must still match the old
  // topic id if that thread secret is still configured.
  ...
  let row = null;
  try {
    const rows = await sbSelect(table, `id=eq.${id}&select=title,jb_ask,result_summary,jb_options&limit=1`);
    row = rows?.[0] ?? null;
  } catch {
    row = null;
  }
  ...
  if (decision && decision.startsWith('option_')) {
    ...
    const optText = row.jb_options[optIdx];
    const nowIso = new Date().toISOString();
    const existingSummary = row?.result_summary ?? null;
    const stampedSummary = `[JB selected: "${optText}" ${nowIso}]${existingSummary ? ` ${existingSummary}` : ''}`;
    await sbPatch(table, `id=eq.${id}`, {
      needs_jb_approval: false,
      status: 'queued',
      result_summary: stampedSummary,
    });
    await telegramAnswerCallbackQuery(token, queryId, 'Selected: ' + optText.slice(0, 40));
