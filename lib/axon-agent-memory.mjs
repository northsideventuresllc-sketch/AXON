 ...
 *
 * NOT ATOMIC: this is a plain read-then-PATCH, not a row-locked RPC (compare
 * fn_axon_append_result_summary's pattern for agent_dispatch). Two concurrent
 * appendAgentMemory calls for the SAME agentId can race and drop one note.
 * Acceptable today because a given persona's own memory is written back by
 * that persona's own task runs, which are not expected to overlap — if a
 * persona ever runs multiple concurrent instances writing memory at once,
 * this needs a real RPC instead.
 *
 * @param {{
 *   agentId: string,
 *   note: string,
