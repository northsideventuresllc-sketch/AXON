...
import { HAIKU_MODEL, GEMINI_MODEL, resolveGeminiModels, todayUtc } from './constants.mjs';
...
async function callGeminiForWisdomPolish(items, geminiKey, geminiBackup, fetchImpl) {
  ...
  const models = resolveGeminiModels(GEMINI_MODEL);
  for (const key of keys) {
    for (const model of models) {
      try {
              ...
              contents: [
                {
                  parts: [
                    {
                      text: `You tighten AXON wisdom units for JB (Northside). Return JSON only: {"items":[{"fingerprint":"...","title":"...","principle":"...","application":"..."}]} — keep fingerprints identical, plain English, no clinical claims.\n\nPolish these wisdom units for operator absorb:\n${JSON.stringify(compact)}`,
                    },
                  ],
                },
              ],
              generationConfig: { maxOutputTokens: 900, temperature: 0.2, responseMimeType: 'application/json' },
            }),
          },
        );
        if (!r.ok) continue;
        ...
        return { items: polished, provider: 'gemini' };
      } catch {
        // try next model/key
      }
    }
...
/**
 * AXON-EVERYWHERE-PROJECT (2026-08-05): AXON-local -> Gemini -> Haiku (last resort) -> heuristic.
 * Same {items, provider} shape as polishWisdomWithHaiku. Decision #598 item 11 / #619.
 *
 ...
 */
export async function polishWisdomTiered(items, { supabaseKey, anthropicKey, geminiKey, geminiBackup } = {}, fetchImpl = fetch) {
  if (!items.length) return { items, provider: 'heuristic' };

  if (supabaseKey) {
    try {
      const text = await callAxonLocal(supabaseKey, WISDOM_POLISH_SYSTEM, wisdomPolishUser(items));
      const result = applyWisdomPolishText(items, text, 'axon-local');
    ...
    try {
      const text = await callAxonV1Cloud(supabaseKey, WISDOM_POLISH_SYSTEM, wisdomPolishUser(items));
      const result = applyWisdomPolishText(items, text, 'axon-v1-runpod');
      if (result) return result;
    } catch {
  ...
  const geminiResult = await callGeminiForWisdomPolish(items, geminiKey, geminiBackup, fetchImpl).catch(() => null);
  if (geminiResult) return geminiResult;

  if (anthropicKey) {
    return polishWisdomWithHaiku(items, anthropicKey, fetchImpl);
  }
...
export async function runWisdomAbsorbLoop(opts = {}) {
  const {
    ...
    geminiBackup = process.env.GEMINI_API_KEY_BACKUP || '',
  ...
  if (!forceHeuristic && (supabaseKey || geminiKey || anthropicKey)) {
    const polished = await polishWisdomTiered(digested, { supabaseKey, anthropicKey, geminiKey, geminiBackup }, fetchImpl);
