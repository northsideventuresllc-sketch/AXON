...
export async function runWisdomAbsorbLoop(opts = {}) {
  ...
  if (!dryRun) {
    ...
    const appliedFindingIds = digested
      .filter((w) => w.source_type === 'research' && w.source_ref)
      .map((w) => w.source_ref);
    if (typeof persistFindingsApplied === 'function' && appliedFindingIds.length) {
      await persistFindingsApplied(appliedFindingIds);
    }
