 ...
 * heuristic. Previously Haiku ran FIRST, ahead of the free Gemini tier and with no
 * AXON-local option at all — inverted from JB's locked tier order (Decision #598 item
 * 11 / #619: AXON local -> Gemini main -> Gemini backup -> Anthropic last). Restructured
 * so Anthropic only runs as the final paid fallback, matching every other AXON surface.
...
export async function synthesizeFindings({
  ...
  if (supabaseKey) {
    try {
      ...
      if (text) {
        ...
        return { ...parsed, _provider: 'axon-local', _model: 'axon-ornith:latest' };
      }
    } catch (err) {
      errors.push(`axon-local: ${err.message}`);
      console.warn(`AXON-local synthesis failed (${err.message}) — trying RunPod (AXON v1)`);
    }

    // AXON-TIER-SYSTEM (2026-08-20, JB direct order): RunPod (AXON v1) tier, right after
    // AXON-local and before Gemini, per the canonical org-wide tier order. No-op (returns
    // null, falls through) until RunPod is deployed.
    try {
      const text = await callAxonV1Cloud(supabaseKey, system, user);
      if (text) {
        const parsed = extractJson(text);
        return { ...parsed, _provider: 'axon-v1-runpod', _model: 'axon-v1' };
      }
    } catch (err) {
      errors.push(`axon-v1-runpod: ${err.message}`);
      console.warn(`RunPod (AXON v1) synthesis failed (${err.message}) — trying Gemini`);
    }
  }
  ...
  if (geminiKey || geminiBackup) {
    try {
      ...
      console.warn(`Gemini synthesis failed (${err.message}) — trying Haiku last resort`);
    }
  } else {
    console.warn('GEMINI_API_KEY missing — trying Haiku last resort for research synthesis');
  }
  ...
  if (anthropicKey) {
    try {
      ...
      console.warn(`Haiku synthesis failed (${err.message}) — using heuristic gather`);
    }
  } else {
    console.warn('ANTHROPIC_API_KEY missing — using heuristic gather for research synthesis');
  }
