 ...
 * LLM calls go through the one locked router chain; heuristic gather is the no-LLM fallback.
 */
...
/**
 * ONE ROUTER (2026-09-06): synthesis walks the single locked chain in
 * lib/axon-router-core.mjs (local -> RunPod -> OpenRouter free -> Gemini ->
 * Anthropic last) instead of the four hand-rolled tiers that used to live here.
 * `_provider` still names whichever lane answered, so `axon_research_findings`
 * rows keep their existing shape, and when the whole chain is unreachable the
 * heuristic gather still runs — research never invents intelligence.
...
export async function synthesizeFindings({
  ...
  const { system, user } = buildSynthesisPrompt(lane, sources, jspaceContext);
  const errors = [];
  ...
  try {
    const out = await generate(supabaseKey, {
      ...
      agentName: 'axon-research-synthesis',
      maxTokens: 2500,
      jsonMode: true,
    });
    const parsed = extractJson(out.text);
    return { ...parsed, _provider: out.source, _model: out.model };
  } catch (err) {
    errors.push(`router chain: ${err.message}`);
    console.warn(`Research synthesis chain failed (${err.message}) — using heuristic gather`);
  }
