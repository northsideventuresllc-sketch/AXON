...
export const SELF_AGENT_NAME = AGENT.REGISTRY_CHECK;
...
export async function fetchAxonRoutines(sbSelect) {
  // Roster truth (Decision #1701, 2026-09-02): the platform column no longer
  // says "axon" for jobs that moved to the Mac mini / pg_cron, so read the
  // whole roster and match on any of its known ids (see routineKeys()).
  return sbSelect(
    'nvg_agent_routines',
    'select=agent_name,routine_id,active,wake_type,wake_config,function_summary,platform,harness,health_status,updated_at&order=agent_name.asc',
  );
}

/**
 * Every id a roster row can be known by in axon_cron_jobs: its routine_id,
 * its source workflow file name (minus .yml/.yaml), and its lower-cased
 * agent_name. Archived rows are excluded — the cron table is expected to
 * forget them.
 */
export function routineKeys(routine) {
  const keys = new Set();
  if (routine?.routine_id) keys.add(String(routine.routine_id).toLowerCase());
  const src = routine?.wake_config?.source_workflow;
  if (src) keys.add(String(src).replace(/\.ya?ml$/i, '').toLowerCase());
  if (routine?.agent_name) keys.add(String(routine.agent_name).toLowerCase());
  return keys;
}

function findRoutineForJob(routines, jobId) {
  const id = String(jobId).toLowerCase();
  return routines.find((r) => r.health_status !== 'archived' && routineKeys(r).has(id)) || null;
}
...
export async function diffRegistry({ cronJobs, routines, ghToken, timeBudgetMs = TIME_BUDGET_MS }) {
  ...
  const cronIds = new Set(cronJobs.map((j) => String(j.id).toLowerCase()));
  const live = routines.filter((r) => r.health_status !== 'archived');

  const findings = [];

  // A — tracked in axon_cron_jobs (has real run history) but no registry row.
  for (const job of cronJobs) {
    if (!findRoutineForJob(routines, job.id)) {
      findings.push({
  ...
  for (const routine of live) {
    // Only AXON-platform rows are expected in axon_cron_jobs; mini / pg_cron /
    // Claude Code rows record their runs elsewhere (nvg_ci_runs, cron.job_run_details).
    if (!/axon/i.test(routine.platform || '')) continue;
    if (![...routineKeys(routine)].some((k) => cronIds.has(k))) {
      findings.push({
  ...
  for (const job of cronJobs) {
    const routine = findRoutineForJob(routines, job.id);
    if (!routine) continue;
...
export function buildReport({ diff, previousFindingIds }) {
  ...
  const headline = `registry check ${diff.generatedAt.slice(0, 10)} — ${diff.findings.length === 0 ? 'all clear' : `${diff.findings.length} finding(s)`}`;
  ...
  return { text: lines.join('\n'), headline, newCount: newOnes.length, stillOpenCount: stillOpen.length };
}
...
// Thin wrapper over the shared lib/slack-post.mjs helper — guarantees the
// required `*<agent> — <headline>*` bold header on every post.
export async function postSlack(headline, body = '') {
  return postAgentOps({ agentName: SELF_AGENT_NAME, headline, body });
}
