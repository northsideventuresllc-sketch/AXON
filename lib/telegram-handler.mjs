...
export async function handleTelegramMessage(cfg, sb, msg) {
  ...
  // Plain conversation lives in JB's private chat only. Inside the NVG Agents
  // group the bot answers slash-commands but never free-text — otherwise every
  // agent topic turns into an AXON chat thread (JB, 2026-09-05).
  const inGroup = cfg.telegramGroupChatId && chatId === String(cfg.telegramGroupChatId);
  if (inGroup && !parseCommand(text)) {
    return null;
  }
  ...
  try {
    if (parsed) {
      ...
      // GROUNDED (2026-09-06): free text is answered from a live snapshot of
      // what actually needs JB, and a real instruction becomes a real job —
      // see lib/axon-jb-chat.mjs.
  ...
  if (reply) {
    ...
    if (!cfg.dryRun) {
      // Answer inside the topic the command came from (group replies without a
      // thread id land in the forum's General topic).
      // JB's own chat hears AXON plainly — no "[AXON — Outreach]" tag in front
      // of the answer (Decision #1696). Group topics keep the tag.
