...
export async function handleTelegramMessage(cfg, sb, msg) {
  ...
  // Instruction-change Reply flow: a free-text message can be JB's note for
  // an "ic:" card he tapped Reply on. Checked before the group's
  // command-only gate below, since the note is free text and may land in
  // the JB Approvals topic inside the group, not just JB's private chat.
