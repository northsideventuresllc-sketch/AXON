 ...
 */
import { answerJbChatMessage } from './axon-jb-chat.mjs';
...
import { SOURCE, shortId, parseNotes } from './constants.mjs';
...
import { handleNvgApproveCallback } from './nvg-approve-telegram.mjs';
import { handleTelegramApprovalReply } from './telegram-approval-reply.mjs';
import {
  handleInstructionChangeCallback,
  tryCaptureInstructionChangeNote,
} from './instruction-change-requests.mjs';
...
import { JB_TEXT } from './telegram-jb-receipts.mjs';
...
async function handleApprove(cfg, sb, lead) {
  ...
  try {
    ...
    if (err instanceof FireHoldError) {
      ...
      const edited = await leadHadDraftEdits(sbSelect, lead.id);
      await logOutreachApproveSignal(sbInsert, lead.id, { operatorId: 'telegram', edited });
      return `AXON is on HOLD — JB needs to flip the Fire/Hold switch before this can send. Approved in the queue, nothing sent yet.`;
    }
    throw err;
  }
...
export const CANONICAL_THREAD_AGENT_MAP = {
  7: 'ARCEUS',
  10: 'AXON Executive',
  31: 'BUILD',
  32: 'CONTENT',
  ...
  217: 'OUTREACH',
  218: 'PR Sweep Bot',
  219: 'NVG Weekend Agent',
  220: 'Weekly Self Reflection',
  221: 'AXON Deep Thinking Agent',
  222: 'Ads Manager',
  223: 'Finance Manager',
  224: 'Inbox Manager',
  225: 'Support',
  ...
  229: 'UX Tester',
  230: 'AXON Research',
  231: 'AXON Model Builder',
  232: 'AXON Training Librarian',
  233: 'AXON Legal Docs',
  234: 'AXON UI Copy',
  235: 'Match Fit Venture Operations',
};

export async function resolveTopicAgent(sbSelect, threadId) {
  if (!threadId) return null;
  if (CANONICAL_THREAD_AGENT_MAP[threadId]) {
    return CANONICAL_THREAD_AGENT_MAP[threadId];
  }
  try {
    const rows = await sbSelect(
      'nvg_agent_routines',
      `telegram_thread_id=eq.${threadId}&select=agent_name&limit=1`
    );
...
export async function handleTelegramMessage(cfg, sb, msg) {
  ...
  // Check reply-to flows before group command filtering:
  // 1. Approval-card reply flow: JB replies to an approval card with notes/instructions/questions.
  // 2. Instruction-change Reply flow: a free-text message for an "ic:" card.
  if (!parseCommand(text)) {
    const approvalReply = await handleTelegramApprovalReply(cfg, sb, msg);
    if (approvalReply) {
    ...
    if (captured) {
      // Plain English, no raw row ID (BUILD-TELEGRAM-JB-UX-AUDIT-0923) — JB
      // can't act on a UUID, and the request is already unambiguous from
      // context (it's the one card he just tapped Reply on).
      const noteConfirmation = captured.target_agent
        ? `Got it — I added your note to the change for ${captured.target_agent}.`
        : 'Got it — I added your note to that change.';
      if (!cfg.dryRun) {
        await telegramSend(cfg.telegramToken, chatId, noteConfirmation, false, {
          threadId: msg.message_thread_id ?? undefined,
        });
      }
  ...
  const inGroup = cfg.telegramGroupChatId && chatId === String(cfg.telegramGroupChatId);
  let topicAgent = null;
  if (inGroup) {
    if (msg.message_thread_id) {
      topicAgent = await resolveTopicAgent(sbSelect, msg.message_thread_id);
    }
    // In group: only reply to commands or messages inside a mapped agent topic thread.
    // Anything else still gets a one-line acknowledgement (P2-ACK) — never a silent drop.
    if (!topicAgent && !parseCommand(text)) {
      if (!cfg.dryRun) {
        try {
          await telegramSend(cfg.telegramToken, chatId, JB_TEXT.nonCardAck, false, {
            threadId: msg.message_thread_id ?? undefined,
            untagged: true,
          });
        } catch (err) {
          console.error('handleTelegramMessage: acknowledgement failed:', err.message);
        }
      }
      return JB_TEXT.nonCardAck;
    }
  }

  const conversation = await getOrCreateConversation(sbSelect, sbInsert, sbPatch, chatId, msg.message_thread_id ?? null);
  ...
  try {
    if (parsed) {
      ...
      // AXON-TELEGRAM-LATENCY-0925: fire the Telegram "typing…" indicator immediately, before
      // any of the (still possibly multi-second, even with the router's interactive per-tier
      // timeouts) work below starts — so JB sees activity instantly instead of a silent chat.
      // Fire-and-forget by design (telegramSendChatAction never awaits its own fetch or
      // throws); never gates the real reply on whether the indicator itself succeeded.
      if (!cfg.dryRun) {
        try {
          telegramSendChatAction(cfg.telegramToken, chatId, 'typing', {
            threadId: msg.message_thread_id ?? undefined,
          });
        } catch {
          // must never block or break the real reply
        }
      }
      ...
      // Grounded fleet intelligence: free text is answered from a live snapshot,
      // and instructions are filed directly for the owning agent.
      const answered = await answerJbChatMessage(cfg, sb, {
        userMessage: text,
    ...
    // Never leave JB's message without an answer (P2-ACK).
    if (!reply) reply = JB_TEXT.chatFallbackAck;
  } catch (err) {
  ...
  if (reply) {
    ...
    if (!cfg.dryRun) {
      // Answer inside the topic the message came from. Untagged plain English.
...
export async function handleTelegramCallback(cfg, sb, callbackQuery) {
  ...
  if (data.startsWith('nvga:')) return handleNvgApproveCallback(cfg, sb, callbackQuery);
  if (data.startsWith('ic:')) return handleInstructionChangeCallback(cfg, sb, callbackQuery);
...
export async function handleOutreachCallback(cfg, callbackQuery) {
  ...
  // raw equality against telegramChatId alone — this Match Fit outreach
  // approve/reject tap is JB-facing too, and the raw check would silently
  // reject a tap from JB's private chat whenever telegramChatId resolved to
  // something else.
  if (!isAuthorizedChat(cfg, chatId)) {
  ...
  try {
    if (action === 'ap') {
      if (isDuplicateApprove(platform, leadId)) {
        reply = `Already approving this ${label} — ignoring the repeat tap.`;
      } else {
