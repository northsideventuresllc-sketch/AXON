...
import { resolveJbTarget } from './jb-route.mjs';
...
export async function loadScaffoldConfig(sbSelect) {
  ...
  return {
    ...
    // SERPAPI-SHARED-QUOTA-STARVING-RESEARCH-0906: the shared SERPAPI_API_KEY
    // is starved by other callers (outreach, etc.), which is why research runs
    // were showing empty queries even when SerpApi itself was healthy. A
    // dedicated SERPAPI_API_KEY_AXON exists in ni_platform_secrets for these
    // research jobs — try it first, fall back to the shared key so a run
    // degrades gracefully instead of going dark if the dedicated one is unset.
    serpApiKey:
      process.env.SERPAPI_API_KEY_AXON
      || (await secret('SERPAPI_API_KEY_AXON'))
      || process.env.SERPAPI_API_KEY
      || (await secret('SERPAPI_API_KEY')),
...
export async function notifyJbUrgent(cfg, text, opts = {}) {
  ...
  // TELEGRAM-ROUTING-FIX-0905: the agent's topic in the NVG Agents group, not
  // JB's private chat. cfg.sbSelect is optional — without it we fall back to
  // the configured chat (private chat when the group is not provisioned).
  if (cfg.sbSelect) {
    const target = await resolveJbTarget(cfg.sbSelect, { agentName: opts.agentName || cfg.agentName });
    if (target.chatId) {
      return telegramSend(cfg.telegramToken, target.chatId, text, cfg.dryRun, { threadId: target.threadId });
    }
  }
