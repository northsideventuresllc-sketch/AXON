 ...
 * (1) hands its output to the AXON Executive Agent via agent_bus,
...
/** Hand a finished piece of research off to another agent (default: AXON Executive Agent). */
...
export async function telegramAlert(sbSelect, text) {
  ...
  if (!token || !chatId) {
    console.log('⚠️ Telegram alert skipped — TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID not reachable');
    return false;
  }
  try {
    ...
    if (!r.ok) console.log(`Telegram alert failed: HTTP ${r.status} — ${(await r.text()).slice(0, 200)}`);
