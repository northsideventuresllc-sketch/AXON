...
/**
 * Send the request card to JB (his private chat only —
 * TELEGRAM-RETIRE-APPROVALS-TOPIC-0924, Decision #2012: the NVG Agents
 * group's "JB Approvals" topic is retired, mirrors the fix in
 * sendBatchNotification in content-machine-telegram.mjs).
 */
export async function sendInstructionChangeRequest(cfg, sb, request) {
  ...
  const result = await telegramSendWithKeyboard(
    cfg.telegramToken,
    cfg.telegramDmChatId ?? cfg.telegramChatId,
    text,
    keyboard,
    false,
    {}
