...
import { verifyMergedToMain } from './axon-live-verify.mjs';
...
//
//     AXON-LIVE-VERIFY-0925 (Learning #9871/#9818): this function runs from
//     inside a local AXON checkout (the Mac mini's nightly cron), so for the
//     'AXON' repo specifically it can ask that local git a real question —
//     is each returned commit actually merged to main — via
//     verifyMergedToMain, instead of a downstream report later guessing from
//     `git log`/`git branch` text and getting it wrong. The other five repos
//     have no established local-clone convention here (see wisdomLoopChecklist
//     / macCronChecklist — only AXON's own path is assumed), so their commits
//     carry `mergedToMain: null` (honestly unverified) rather than a guess.
// ---------------------------------------------------------------------------
export async function fetchRecentCommits(
  ghToken,
  { repos = NVG_REPOS, sinceHours = 24, localAxonRepoCwd = process.cwd() } = {},
) {
    ...
    try {
      ...
      results.push({
        ...
        commits: data.slice(0, 10).map((c) => {
          ...
          const mergedToMain =
            repo === 'AXON' ? verifyMergedToMain(sha, { cwd: localAxonRepoCwd }) : { verified: false, merged: false, reason: 'no local clone convention for this repo' };
          return {
            sha,
...
export async function alertTelegramUrgent(sbSelect, text) {
  // TELEGRAM-ROUTING-FIX-0905: AXON Executive's own topic in the NVG Agents group.
  try {
    const ok = await sendToJb(sbSelect, text, { agentName: 'AXON Executive' });
    return { ok, reason: ok ? undefined : 'telegram bot or chat not available' };
  } catch (err) {
...
export async function registerCron(serviceKey, { status, summary, overBudget }) {
  ...
  try {
    ...
    // Also stamp nvg_agent_routines.last_fired_at for this agent — a real nightly run
    // just happened, but the fleet-liveness table only ever got written by the manual
    // dashboard "Fire" button (lib/axon-roster-fire.mjs), never by this scheduled path.
    // Found by PULSE 2026-09-09 (ticket PULSE-LASTFIRED-COSTLEDGER-AXON-GAP-0909):
    // AXON Executive genuinely runs nightly but last_fired_at stayed NULL forever.
    // Best-effort — never let this block the axon_cron_jobs result above.
    try {
      await fetch(
        `${SUPABASE_URL}/rest/v1/nvg_agent_routines?agent_name=eq.${encodeURIComponent(AGENT_NAME)}`,
        {
          method: 'PATCH',
          headers: {
            apikey: serviceKey,
      ...
      console.warn('registerCron: nvg_agent_routines last_fired_at stamp failed:', err.message);
    }
