 ...
 *      (to_agent='AXON Executive Agent'), and writes its own summary back
 *      out (from_agent='AXON Executive Agent') the same way every other
...
export const AGENT_NAME = 'AXON Executive Agent';
...
// Same publishable/anon key every other NVG agent uses to call slack-post — not a secret,
// it is the project's public REST anon key (Supabase RLS gates the actual writes).
