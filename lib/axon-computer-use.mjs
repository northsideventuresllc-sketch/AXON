 ...
 * the first real run.
 *
 * DEPENDENCY (confirmed 2026-08-27): `cliclick` is built + installed on the mini at
 * ~/.local/bin/cliclick (no Homebrew on that machine — built from source instead, with a
 * local patch for a macOS-26 CGWindowListCreateImage removal in its color-picker action,
 * confined to that one unused action). cliclickCmd() below prefixes PATH on every
 * invocation rather than assuming the job runner's own PATH includes ~/.local/bin — the
 * runner is a launchd-spawned python3 daemon (com.nvg.mini-runner), which typically
 * doesn't inherit a login shell's PATH. Scroll and key combos use AppleScript/System
 * Events instead, which ships with macOS and doesn't depend on cliclick.
 *
 * REMAINING BLOCKER (confirmed 2026-08-27, NOT resolved by this code): Accessibility
 * permission has not been granted to the process actually driving clicks/keystrokes.
 * TCC grants attach to the calling process, not to cliclick itself — since jobs run via
 * the mini runner's /usr/bin/python3, that's what needs the grant, and a background
 * launchd daemon won't get the normal GUI permission prompt a terminal app would. Until
 * this is granted (System Settings → Privacy & Security → Accessibility, added by hand
 * on the mini), `screenshot`/read-only actions will work but click/type/key/scroll will
 * silently do nothing. This is a physical-machine step outside what any code change here
 * can fix — do not attempt to route around it (e.g. by weakening the credential/scope
 * guardrails in the system prompt); it must be granted deliberately, on the mini, by JB
 * or whoever has hands on that machine.
 */
...
/**
 * cliclick was built from source into ~/.local/bin (no Homebrew on the mini — confirmed
 * 2026-08-27), not a standard PATH location. The mini's job runner is a launchd-spawned
 * python3 daemon (com.nvg.mini-runner), which typically has a minimal PATH that won't
 * include a user's ~/.local/bin — so every bare `cliclick` call risked a silent
 * "command not found". Prefixing PATH on each invocation avoids depending on the
 * runner's environment or hardcoding an absolute path that could drift.
 */
