...
export async function handleTelegramApprovalReply(cfg, sb, msg) {
  ...
  if (dispatchRow) {
    ...
    try {
      await sbPatch('agent_dispatch', `id=eq.${dispatchRow.id}`, {
        ...
        /* Session stays open for back-and-forth chat */
  ...
  // Trigger LLM back-and-forth instead of a static "Noted" response
  const { axonChatReply } = await import('./axon-telegram-chat.mjs');
  let reply = `Noted for ${targetOwner}: ${shortTitle}. What else do you need?`;
  try {
    const historyRows = await sbSelect('axon_telegram_messages', `conversation_id=eq.${originalMsg.conversation_id || 'a1d8c586-ce8e-4736-997d-648bb33e2872'}&order=created_at.desc&limit=16`);
    const history = (historyRows || []).reverse().filter(h => h.role === 'user' || h.role === 'assistant');
    
    // Provide the dispatch context to the LLM
    const contextFacts = `The user is replying to an active agent dispatch:
    Code: ${dispatchRow?.code || 'N/A'}
    Title: ${dispatchRow?.title || 'N/A'}
    Owner: ${targetOwner}
    Current Ask: ${originalMsg.content}`;
  ...
  if (!cfg.dryRun) {
    try {
      const { telegramSend } = await import('./telegram.mjs');
