...
/** Match Fit content is NEVER AXON/CM6 Telegram (2026-07-13/14/16 lock). */
export function allowMatchFitContentTelegram() {
  return (
...
export function isMatchFitBrand(slug) {
  return String(slug || '').toLowerCase() === 'match-fit';
}

/** True when a post must not be sent/shown on AXON Telegram. */
export function isMatchFitContentBlocked(postOrSlug) {
  if (allowMatchFitContentTelegram()) return false;
  const slug = typeof postOrSlug === 'string' ? postOrSlug : postOrSlug?.brand_slug;
  return isMatchFitBrand(slug);
}
...
export async function fetchPendingPosts(sbSelect, { batchId = null } = {}) {
  ...
  if (!allowMatchFitContentTelegram()) {
    // CM6 must not surface Match Fit drafts on @northsideaxonbot
    filter += '&brand_slug=neq.match-fit';
  }
...
export async function findPostByShortId(sbSelect, sid, { pendingOnly = true } = {}) {
  ...
  const post = (rows || []).find((r) => shortId(r.id) === sid || r.id === sid) || null;
  if (post && isMatchFitContentBlocked(post)) return null;
  return post;
}
...
export async function handleContentCallback(cfg, sb, callbackQuery) {
  ...
  try {
    ...
    console.error('handleContentCallback failed:', err.message);
    reply = 'That action failed on my end. Try again in a moment.';
  }
...
export async function sendBatchNotification(cfg, sb, posts) {
  ...
  if (isMatchFitContentBlocked(posts[0])) {
    console.log(
      `Skip Match Fit batch ${posts[0]?.batch_id || posts[0]?.id} — AXON CM6 MF content blocked`,
    );
    return null;
  }
