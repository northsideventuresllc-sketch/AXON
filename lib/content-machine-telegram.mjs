...
export async function sendBatchNotification(cfg, sb, posts) {
  ...
  if (!cfg.dryRun) {
    // TELEGRAM-APPROVALS-TO-DM-0924: approval cards go to JB's private chat
    // now, not the JB Approvals topic (cfg.telegramChatId is DM-first — see
    // config.mjs). Only attach message_thread_id when the resolved chat is
    // actually the group (defensive: a DM has no forum topics, so sending a
    // stale/still-provisioned telegramApprovalsThreadId against a DM chat_id
    // would fail the Telegram API call outright).
