...
/** Match Fit content is NEVER AXON/CM6 Telegram (2026-07-13/14 lock). */
function allowMatchFitContentTelegram() {
  return (
...
export async function sendBatchNotification(cfg, sb, posts) {
  ...
  if (!cfg.dryRun) {
    // TELEGRAM-RETIRE-APPROVALS-TOPIC-0924 (Decision #2012): approval cards
    // go to JB's private chat now, never the retired group "JB Approvals"
    // topic — previously this sent to cfg.telegramChatId/telegramApprovalsThreadId,
    // which resolve to the group whenever it and the approvals-thread secret
    // are both still provisioned, silently misrouting content-batch approvals
    // to the dead topic even after Decision #2012.
    await telegramSendWithKeyboard(cfg.telegramToken, cfg.telegramDmChatId ?? cfg.telegramChatId, text, keyboard, false, {});
  } else {
