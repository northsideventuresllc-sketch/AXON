/**
 * AXON Self-Research — three-area build-plan mode, plus the daily competitor lane.
 ...
 * See buildCompetitorArea below.
 *
 * BUILD-COMPETITOR-RESEARCH-STARVED-FIX-0923 (JB direct, high heat): the Mon/Wed/Fri
 * gate plus running the lane LAST of 4 areas meant the per-run time budget routinely
 * hit before competitor ever ran (logged "3/4 done, stopped at competitor_gap_build"
 * on 09-21 and 09-23), and the lane's own briefing write was hardcoded to 0 — real
 * findings never reached the daily briefing surface. Fixed here: the competitor area
 * now runs FIRST every day (never gated, never starved by the budget check landing on
 * it last), and every completed (non-skipped) area writes a real briefing item via
 * applyBriefingUpdatesRaw so briefing_items_added reflects what actually shipped.
 *
...
import { writeResearchRunLabLog, applyBriefingUpdatesRaw } from './axon-research-core.mjs';
 ...
 *
 * BUILD-COMPETITOR-RESEARCH-STARVED-FIX-0923 / BUILD-COMPETITOR-RESEARCH-DAILY-0923
 * (JB direct 2026-09-23): the old Mon/Wed/Fri gate meant rival research silently
 * produced nothing on 4 of every 7 days. The lane now runs every day — isCompetitorLaneDay
 * is kept (and always true) only so callers/tests that gate on it keep working; it is no
 * longer a real filter.
 */
export const COMPETITOR_LANE_ID = 'competitor_gap_build';

/** Always true — the competitor lane now runs daily (BUILD-COMPETITOR-RESEARCH-DAILY-0923). */
export function isCompetitorLaneDay() {
  return true;
}
...
export async function runThreeAreaResearch({
  ...
  // Competitor lane (BPA A2, BUILD-COMPETITOR-RESEARCH-STARVED-FIX-0923): runs every
  // day now, and goes FIRST in the areas list so the per-run time budget — which is
  // checked before each area — can never starve it by landing on it last.
  ...
  try {
    ...
    if (venture) {
      areas.unshift(buildCompetitorArea(venture));
    } else {
  ...
  // BUILD-COMPETITOR-RESEARCH-STARVED-FIX-0923 (c): completed areas previously never
  // reached the briefing surface at all — briefingItemsAdded was hardcoded to 0. Every
  // non-skipped area (competitor included — these are the "rival notes" that were
  // silently going nowhere) now writes a real item via the same door the older lane
  // (axon-research-core.mjs) already uses.
  const briefingUpdates = [];
  if (!dryRun) {
    for (const r of results) {
      if (r.result.skipped) continue;
      const emoji = r.area.id === COMPETITOR_LANE_ID ? '🏆' : '🔬';
      briefingUpdates.push({
        action: 'add',
        title: `${emoji} ${r.area.label}: ${String(r.result.finding || '').slice(0, 80)}`,
        content: [
          r.result.plain_english || r.result.finding || '',
          r.result.build_plan?.what_to_build ? `Build: ${r.result.build_plan.what_to_build}` : '',
        ]
          .filter(Boolean)
          .join('\n'),
        priority: r.result.build_plan?.priority || 'medium',
        source: 'axon',
      });
    }
    if (briefingUpdates.length) {
      await applyBriefingUpdatesRaw(sbSelect, sbInsert, sbPatch, briefingUpdates, operatorId).catch((err) =>
        console.log(`⚠️ briefing update failed: ${err.message}`)
      );
    }
  }
