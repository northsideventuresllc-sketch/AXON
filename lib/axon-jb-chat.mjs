 ...
 * @param {{ userMessage: string, history?: Array, now?: Date, generate?: Function, topicAgent?: string|null }} opts
 */
export async function answerJbChatMessage(cfg, sb, { userMessage, history = [], now = new Date(), generate, topicAgent = null }) {
  ...
  const facts = await buildJbChatContext(sbSelect, { now, pipelineContext, pipelineFailed });
  ...
  if (kind === 'instruction') {
    const { row, reply } = await fileJbInstruction(sbInsert, userMessage, { now, sbSelect, owner: topicAgent });
  ...
  const dispatchMatch = reply.match(/\[DISPATCH:\s*([^\]]+)\]/);
  if (dispatchMatch) {
    const agentToDispatch = dispatchMatch[1].trim();
    // Dispatch the request to the agent directly!
    const { row, reply: dispatchReply } = await fileJbInstruction(sbInsert, userMessage, { now, sbSelect, owner: agentToDispatch });
    const finalReply = reply.replace(/\[DISPATCH:\s*[^\]]+\]/, '').trim() || dispatchReply;
    return { reply: finalReply, route: 'model_dispatched', dispatch: row, facts };
  }
