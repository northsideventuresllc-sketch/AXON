...
export const MAX_OUTREACH_LEADS_PER_CALL = 20;

/**
 * Push a batch of leads through `sendLead`, capped at `maxPerCall`. One lead's send failing
 * doesn't abort the rest of the batch — returns counts so the route can report partial
 * progress instead of losing already-pushed leads behind a bare 500.
 */
export async function pushOutreachLeadsBatch(leads, sendLead, maxPerCall = MAX_OUTREACH_LEADS_PER_CALL) {
  const toSend = leads.slice(0, maxPerCall);
...
const recentApproveKeys = new Map();
const APPROVE_DEDUP_TTL_MS = 30_000;

/**
 * True if this platform+leadId was already approved within the dedup window — Telegram
 * replays a callback update until the webhook answers 2xx (same pattern as the stale-callback
 * fix in lib/telegram.mjs), so a slow first response can otherwise queue the same lead twice.
 * Caller should skip the Match Fit call on a duplicate rather than re-queueing the lead.
 */
export function isDuplicateApprove(platform, leadId, now = Date.now()) {
  const key = `${platform}:${leadId}`;
  for (const [k, ts] of recentApproveKeys) {
    if (now - ts > APPROVE_DEDUP_TTL_MS) recentApproveKeys.delete(k);
  }
  if (recentApproveKeys.has(key)) return true;
  recentApproveKeys.set(key, now);
  return false;
}
