...
/**
 * Full request-handling logic for the posting-confirmation webhook, extracted from the
 * route handler (app/api/axon/match-fit/posting-confirmation/route.ts) so the 401/400/
 * 500/happy-path branches are exercised by a real test instead of only the pure
 * validation/builder helpers above. `addNotification` is injected so tests can stub it
 * without touching the real preferences store.
 * Returns { status, body } — the route maps this straight onto NextResponse.json.
 */
export async function handlePostingConfirmationRequest({ headerSecret, envSecret, rawBody, addNotification }) {
  ...
  try {
    ...
    return { status: 400, body: { ok: false, error: 'Malformed JSON body' } };
  }

  const validation = validatePostingConfirmationPayload(parsedBody);
  if (!validation.ok || !validation.data) {
    return { status: 400, body: { ok: false, error: validation.error ?? 'Invalid payload' } };
  }
  ...
  try {
    ...
    return { status: 200, body: { ok: true, batchId, postsRecorded: posts.length } };
  } catch (err) {
    console.error('Match Fit posting-confirmation webhook failed:', err);
    return {
      status: 500,
      body: { ok: false, error: err instanceof Error ? err.message : 'Failed to record confirmation' },
    };
  }
}
