...
async function callChatModel(cfg, system, messages, maxTokens = 900) {
  const local = await callAxonLocal(cfg.supabaseKey, system, messages).catch((e) => {
    console.error('[axon-telegram-chat] tier0 AXON-local failed:', e?.message || e);
    return null;
  });
  ...
  for (const key of [cfg.geminiKey, cfg.geminiBackup].filter(Boolean)) {
    try {
      ...
      if (text) return text;
      console.error('[axon-telegram-chat] tier1 Gemini returned empty, trying next key/tier');
    } catch (e) {
      console.error('[axon-telegram-chat] tier1 Gemini threw:', e?.message || e);
    }
  }
