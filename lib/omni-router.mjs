 ...
 *   - Cursor is NOT wired in and is not planned as a callWithFailover provider — Cursor
 *     has no server-side completions API to call (it is an IDE product; its agent mode
 *     runs inside the editor, not as a hostable HTTP endpoint). Documented as a known
 *     non-goal in docs/omni-router.md, not a silently dropped task.
 *   - DeepSeek IS wired in this increment (see PROVIDERS / callDeepSeek below) but is
 *     NOT added to DEFAULT_ORDER — no live call site uses this module yet (see next
 *     bullet), so changing DEFAULT_ORDER has zero production effect either way; left
 *     out of the default to keep this increment's diff minimal. A caller opts in via
 *     `opts.order: [...DEFAULT_ORDER, 'deepseek']` or any order that includes it.
...
export const PROVIDERS = [
  ...
  { id: 'deepseek', label: 'DeepSeek', requiresKey: true, envKey: 'DEEPSEEK_API_KEY' },
];
...
async function callDeepSeek(system, user, opts = {}) {
  const apiKey = opts.deepseekApiKey || process.env.DEEPSEEK_API_KEY;
  if (!apiKey) throw new Error('DEEPSEEK_API_KEY missing');
  const model = opts.deepseekModel || process.env.DEEPSEEK_MODEL || 'deepseek-chat';
  // OpenAI-compatible chat-completions shape — https://api-docs.deepseek.com/
...
const BUILTIN_CALLERS = { local: callLocal, claude: callClaude, gemini: callGemini, deepseek: callDeepSeek };
