...
export function classifyJbMessage(text) {
  ...
  if (HEDGES.test(raw) && namesWork) return 'clarify';
  // "Fix the cron tab?" is JB checking, not ordering.
  if (raw.includes('?') && (namesWork || isVerbStart)) return 'clarify';
  if (raw.includes('?')) return 'chat';
  if (!please && QUESTION_OPENERS.test(firstWord)) return 'chat';
  if (isVerbStart || (please && namesWork)) return 'instruction';
  return 'chat';
}
...
/**
 * AXON-TELEGRAM-DISPATCH-OWNER-VALIDATION-0916 — the [DISPATCH: AgentName]
 * protocol (2026-09-15, commit f97ac2fc) lets the model's own free-text
 * output name a ticket owner directly. Before this fix, buildDispatchRow()
 * wrote that string straight into agent_dispatch.owner with no check against
 * ROSTER — a hallucinated or unwired name (the system prompt itself
 * advertises "AXON Research", which isn't in ROSTER and has no consumer that
 * reads agent_dispatch for that name) became a permanent dead-letter ticket,
 * same failure shape as the REACH/DESK incidents in nvg-agent-comms.
 * Returns the canonical ROSTER spelling on a case-insensitive match, else null.
 */
export function matchRosterName(candidate) {
  const c = String(candidate || '').trim();
  if (!c) return null;
  return ROSTER.find((name) => name.toLowerCase() === c.toLowerCase()) || null;
}
...
export function buildDispatchRow(text, { now = new Date(), owner = null } = {}) {
  ...
  const chosen = matchRosterName(owner) || pickOwner(words);
...
export async function fileJbInstruction(sbInsert, text, { now = new Date(), sbSelect = null, owner = null } = {}) {
  const row = buildDispatchRow(text, { now, owner });
