 ...
 *   7. AX-SUBAGENT-MEMORY-FIELD-0904 fallback: this agent's OWN self-curated
 *      memory (`lib/axon-agent-memory.mjs` — `axon_venture_agents.config.memory`,
 *      no new table). Distinct from #6: wisdom is fleet-wide and consolidated,
 *      this is per-persona and written back by that same agent after its own
 *      tasks via appendAgentMemory(). Own budget, same reason as #6.
 *
...
import { formatAgentMemoryBlock } from './axon-agent-memory.mjs';
...
export async function buildAgentBootContext(agentId) {
  ...
  const memoryEntries = Array.isArray(agent.config?.memory) ? agent.config.memory : [];
  const memoryBlock = formatAgentMemoryBlock(memoryEntries);
