 ...
 * @param {{capabilityClass: string, quota?: Record<string, number>}} ctx
 * @returns {Array} ranked [{lane, score, reasons[]}], best first
 */
export function scoreLanes(lanes, { capabilityClass, quota = {} } = {}) {
  const scored = [];
 ...
 *   agentRole, hasMini, isComputerUse, requestId, venture
 * @returns {Promise<{reply: string, route: string, decisionId: string|null, capabilityClass: string}>}
 */
export async function routeChat(supabaseKey, args = {}) {
  ...
  if (mode === 'fixed' && (laneOverride || fixedOrder)) {
    ...
    if (!ranked.length) ranked = scoreLanes(all, { capabilityClass });
  } else {
    ranked = scoreLanes(all, { capabilityClass });
  }
  ...
  for (const cand of ranked) {
    try {
      const out = await executeLane(supabaseKey, cand.lane, messages, { hasMini });
      if (out.unavailable) {
