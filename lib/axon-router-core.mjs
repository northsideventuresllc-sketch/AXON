...
async function executeChainTier(
  supabaseKey,
  { tier, route, model, fallbackModels = [], configuredModels = [], pin = null, accountId, maxTokens = 1024, jsonMode = false, hasMini = false },
  messages,
) {
  ...
  if (tier === 'local') {
    ...
    const orderLocal = (live) => {
      if (!live) return configuredIds;
      const installed = [...live].sort((a, b) => Number(b.startsWith('axon-')) - Number(a.startsWith('axon-')));
      return buildCandidates({ live: installed, configured: configuredIds, pinsFirst: true, provider: 'ollama' });
    };
    let candidates = orderLocal(await getLiveModels('ollama', { supabaseKey, base, cachedOnly: true }));
