...
/**
 * Splits raw `nvg_rolling_tasks` rows into Repeating (daily, then weekly for the
 * given weekday) and Non-repeating (open + monthly-due-today + completed) groups.
 * `agentic_question` rows are out of scope for this LOCKED format (not a task the
 * skill's three surfaces cover) and are silently excluded, never merged in.
 *
...
export function groupRollingTasks(rows, today = {}) {
  ...
  const dow = Number.isInteger(today.dayOfWeek) ? today.dayOfWeek : isoWeekday(new Date());
  ...
  const weeklyRepeating = list.filter(
    (r) =>
      r &&
  ...
  // Daily block first, then weekly for today — matches the skill's locked table order.
