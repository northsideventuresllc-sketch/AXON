 ...
 *
 * PREREQUISITE, CONFIRMED LIVE 2026-09-08 (read-only mini probe): Accessibility permission
 * is NOT yet granted to the mini job runner's calling process — cliclick itself reports
 * "Accessibility privileges not enabled. Many actions may fail." Screenshots will work;
 * every click/type/key/scroll action will silently no-op until this is granted on the mini
 * (System Settings → Privacy & Security → Accessibility). Do not run this expecting a real
 * pass until that is done — running it before then will burn a real Anthropic API loop and
 * end in outcome != 'complete' with clicks that did nothing.
 *
